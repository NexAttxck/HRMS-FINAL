...
if($_SERVER["REQUEST_METHOD"]==="POST"){$a=$_POST["_action"]??"";
    if($a==="save"){$id=(int)($_POST["id"]??0);
        if($id){
            $targetUser = DB::fetchOne("SELECT role FROM `user` WHERE id=?", [$id]);
            if ($targetUser && $targetUser['role'] === 'Super Admin' && $id !== Auth::id()) { Auth::flash("error", "Cannot edit another Super Admin."); header("Location: ".url("user_management")); exit; }
            ...
            // Auto-generate a unique employee number
            $empYear = date("Y");
            $empCount = (int)DB::fetchScalar("SELECT COUNT(*) FROM employee") + 1;
            $newEmpNo = sprintf("EMP-%s-%04d", $empYear, $empCount);
            while (DB::fetchScalar("SELECT COUNT(*) FROM employee WHERE employee_no=?", [$newEmpNo])) {
                $empCount++;
                $newEmpNo = sprintf("EMP-%s-%04d", $empYear, $empCount);
            }
            $hireDate = date('Y-m-d'); // Hire date = account creation date
            $newEmpRowId = DB::insert("INSERT INTO employee (user_id,employee_no,first_name,last_name,email,department_id,status,employment_type,hire_date,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",[$newUid,$newEmpNo,$firstName,$lastName,$_POST["email"],$deptId,'Probationary','Full-Time',$hireDate,time(),time()]);
    ...
    if($a==="reset_password"){$rpId=(int)$_POST["id"];$newPw=trim($_POST["new_password"]??"");
        $targetUser = DB::fetchOne("SELECT role FROM `user` WHERE id=?", [$rpId]);
        if ($targetUser && $targetUser['role'] === 'Super Admin' && $rpId !== Auth::id()) { Auth::flash("error", "Cannot reset another Super Admin's password."); header("Location: ".url("user_management")); exit; }
    ...
    if($a==="delete"){$delUid=(int)$_POST["id"];
        if ($delUid === Auth::id()) { Auth::flash("error", "You cannot delete your own account."); header("Location: ".url("user_management")); exit; }
        $targetUser = DB::fetchOne("SELECT role FROM `user` WHERE id=?", [$delUid]);
        if ($targetUser && $targetUser['role'] === 'Super Admin') { Auth::flash("error", "Cannot delete a Super Admin account."); header("Location: ".url("user_management")); exit; }
        Auth::audit('Delete User','User Management',$delUid);
...
$users = DB::fetchAll("
    SELECT u.id, u.username, u.email, u.role, u.status, u.department_id, d.name as dept_name, NULL as orphan_emp_id
    FROM `user` u LEFT JOIN department d ON u.department_id=d.id
    UNION ALL
    SELECT NULL as id,
        CONCAT(e.first_name,' ',e.last_name) as username,
        e.email, 'Employee' as role, 'No Account' as status,
        e.department_id, d.name as dept_name, e.id as orphan_emp_id
    FROM employee e
...
<div><label style="display:block;font-size:12px;font-weight:600;color:var(--text-muted);margin-bottom:4px;text-transform:uppercase;">Username</label><input type="text" name="username" id="um_username" oninput="suggestEmail(this.value)" value="<?php echo e($es["username"]??""); ?>" required style="width:100%;padding:9px 12px;background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px;"></div>
<div><label style="display:block;font-size:12px;font-weight:600;color:var(--text-muted);margin-bottom:4px;text-transform:uppercase;">Email</label><input type="email" name="email" id="um_email" value="<?php echo e($es["email"]??""); ?>" placeholder="username@cura.ph" required style="width:100%;padding:9px 12px;background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px;"></div>
<div><label style="display:block;font-size:12px;font-weight:600;color:var(--text-muted);margin-bottom:4px;text-transform:uppercase;">Password <?php echo $es?"(blank=unchanged)":""; ?></label><div style="position:relative;"><input type="password" name="password" id="um_pass" style="width:100%;padding:9px 36px 9px 12px;background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px;"><button type="button" onclick="toggleVisibility('um_pass')" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text-muted);cursor:pointer;padding:4px;">&#128065;</button></div></div>
...
<td><?php
    if ($u['status'] === 'No Account') {
        echo '<span style="padding:2px 10px;background:rgba(251,140,0,0.15);color:#fb8c00;border-radius:12px;font-size:12px;font-weight:600;border:1px solid rgba(251,140,0,0.3);">No Account</span>';
    } else {
        echo '<span class="badge '.($u['status']==='Active'?'badge-success':'badge-danger').'">'.e($u['status']).'</span>';
    }
?></td>
<td style="text-align:right;display:flex;gap:6px;justify-content:flex-end;align-items:center;">
<?php if ($u['status'] === 'No Account'): ?>
<td style="text-align:right;display:flex;gap:6px;justify-content:flex-end;align-items:center;">
    <a href="<?php echo url('user_management',['action'=>'create']).'&prefill_emp='.$u['orphan_emp_id']; ?>" class="btn btn-sm btn-accent" style="padding:3px 10px;">
        + Create Account
    </a>
</td>
<?php elseif($u["role"] !== "Super Admin" || $u["id"] == Auth::id()): ?>
...
<td colspan="6" style="padding:12px 20px;">
    <form method="POST" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        ...
        <div style="position:relative;display:inline-block;"><input type="password" name="new_password" id="rp_pass_<?php echo $u['id']; ?>" placeholder="New password (min 6 chars)" required minlength="6" style="padding:7px 32px 7px 12px;background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px;min-width:220px;"><button type="button" onclick="toggleVisibility('rp_pass_<?php echo $u['id']; ?>')" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text-muted);cursor:pointer;padding:4px;">&#128065;</button></div>
...
function toggleVisibility(id) {
    ...
    if(el && el.type === 'password') { el.type = 'text'; } else if(el) { el.type = 'password'; }
}
function suggestEmail(username) {
    var emailField = document.getElementById('um_email');
    if (!emailField || emailField.dataset.manuallyEdited === '1') return;
    if (!username.trim()) { emailField.value = ''; return; }
    var slug = username.trim().toLowerCase().replace(/\s+/g, '.');
    emailField.value = slug + '@cura.ph';
}
document.addEventListener('DOMContentLoaded', function() {
    var ef = document.getElementById('um_email');
    if (ef) {
        ef.addEventListener('input', function() { this.dataset.manuallyEdited = '1'; });
    }
});
</script>
